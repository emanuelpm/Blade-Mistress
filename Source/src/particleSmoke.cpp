...
//********************************************************************************
ParticleSmokeType::ParticleSmokeType(int maxParticles, char *textureName, float delta, float yD)
{
   ...
   puma->m_pd3dDevice->CreateVertexBuffer( (numOfParticles + 1) * 
	...
	particleArray = new ParticleSmoke[numOfParticles];
...
//********************************************************************************
ParticleSmokeType::~ParticleSmokeType()
{
...
//********************************************************************************
void ParticleSmokeType::SetEmissionPoint(D3DXVECTOR3 point)
{
...
//********************************************************************************
void ParticleSmokeType::SetEmissionBox  (D3DXVECTOR3 least, D3DXVECTOR3 most)
{
...
extern char testPT2Text[1024];

//********************************************************************************
void ParticleSmokeType::Emit(int amount, D3DXCOLOR color, float scale)
{
	int exit = FALSE;
...
//			DebugOutput("E5 ");
			...
			particleArray[lastParticleUsed].size  = scale;
...
//********************************************************************************
void ParticleSmokeType::Tick(float timeDelta)
{
	for (int i = 0; i < numOfParticles; ++i)
	{
...
//			particleArray[i].color = D3DCOLOR_ARGB(al, 000, 255, al);
			...
			particleArray[i].m_vPos.x += particleArray[i].m_vVel.x * timeDelta * particleArray[i].size;
			particleArray[i].m_vPos.y += particleArray[i].m_vVel.y * timeDelta * particleArray[i].size;
			particleArray[i].m_vPos.z += particleArray[i].m_vVel.z * timeDelta * particleArray[i].size;

			particleArray[i].m_vVel.y += yDelta * timeDelta	* scaleDelta;

			particleArray[i].size     += particleArray[i].size * scaleDelta * timeDelta;
		}
	}
...
//********************************************************************************
void ParticleSmokeType::PrepareToDraw(void)
{
...
//********************************************************************************
void ParticleSmokeType::Draw(D3DXMATRIX matTrans)
{
//	camAngle *= 1;
...
//   puma->m_pd3dDevice->SetRenderState( D3DRS_POINTSIZE,     FtoDW(particleSize) ); // size!
	...
	for (int i = 0; i < numOfParticles; ++i)
	{
		...
		if (particleArray[i].life >= 0)
		{
			...
			D3DXMatrixTranslation( &localMat[0], 0,-0.5 * particleArray[i].size,-0.5 * particleArray[i].size);
			D3DXMatrixTranslation( &localMat[1], 0,-0.5 * particleArray[i].size, 1.5 * particleArray[i].size);
			D3DXMatrixTranslation( &localMat[2], 0, 1.5 * particleArray[i].size,-0.5 * particleArray[i].size);
			...
			pMyVertices[vIndex].color = tempColor; //0xffffffff;
