...
#define D3DFONT_VERTCENTERED 0x0008
#define D3DFONT_RIGHTSIDE    0x0010
...
public:
    ...
    HRESULT DrawText( RECT rect, RECT *clipRect, DWORD dwColor, 
                      TCHAR* strText, DWORD dwFlags=0L );
    HRESULT DrawTextScaled( FLOAT x, FLOAT y, FLOAT z, 
                            FLOAT fXScale, FLOAT fYScale, DWORD dwColor, 
//                            TCHAR* strText, DWORD dwFlags=D3DFONT_FILTERED );
                            TCHAR* strText, DWORD dwFlags=0L );
    HRESULT Render3DText( TCHAR* strText, DWORD dwFlags , DWORD dwColor );
    
    // Function to get extent of text
    HRESULT GetTextExtent( TCHAR* strText, SIZE* pSize );
	 int GetTextFitNum( TCHAR* strText, float size, int wordWrap );

    DWORD GetTextHeight(void) {  return m_dwFontHeight; };
	 int   RealTextHeight(void);
