...
VOID UpdateUI( HWND hDlg )
{
    ...
    {
        strcpy( strExcepted, "For security reasons, background exclusive "
    ...
    else
    {
        if( bForeground )
        {
            strcpy( strExcepted, "Foreground cooperative level means that the "
        ...
        {
            strcpy( strExcepted, "Background cooperative level really means "
        ...
        if( bExclusive )
        {
            strcat( strExcepted, "Exclusive mode prevents other applications from "
                                 ...
                                 "CTRL+ALT+DEL and, on Windows 95 and Windows 98, "
                                 "ALT+TAB.\n\n" );
        }
        else
        {
            strcat( strExcepted, "Nonexclusive mode means that other applications "
                                 "can acquire device in exclusive or nonexclusive mode. " );

            if( bDisableWindowsKey )
            {
                strcat( strExcepted, "The Windows key will also be disabled so that "
        ...
        if( bImmediate )
        {
            strcat( strExcepted, "Immediate data is a snapshot of the current "
        ...
        {
            strcat( strExcepted, "Buffered data is a record of events that are stored "
        ...
        strcat( strExcepted, "The sample will read the keyboard 12 times a second. "
