...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Create( HINSTANCE hInstance, int startInWindow )
{
    HRESULT hr;
    ...
    if( FAILED( hr = BuildDeviceList(startInWindow) ) )
    {
        SAFE_RELEASE( m_pD3D );
    ...
    if( m_hWnd == NULL)
    {
        ...
        m_dwWindowStyle = WS_POPUP|WS_CAPTION|
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::BuildDeviceList(int startInWindow)
{
	 char tempText[1024];
    ...
    BOOL bHALIsSampleCompatible = FALSE;
	 ...
	 FILE *fp = fopen("VideoCardInfo.txt","w");
	 if (fp)
	 {
		 fprintf(fp,"PUMA video system enumeration\n\n");
		 fclose(fp);
	 }
    ...
    for( UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++ )
    {
        ...
        pAdapter->dwCurrentDevice = 0;
		  ...
		  sprintf(tempText, "%s\n%s\n",
			      pAdapter->d3dAdapterIdentifier.Driver,
			      pAdapter->d3dAdapterIdentifier.Description );
		  aLog.Log(tempText);

		  sprintf(tempText, "Desktop is %dx%d, %dhz, Format %d\n",
			      pAdapter->d3ddmDesktop.Width,
			      pAdapter->d3ddmDesktop.Height,
			      pAdapter->d3ddmDesktop.RefreshRate,
        ...
        for( UINT iMode = 0; iMode < dwNumAdapterModes; iMode++ )
        {
            ...
            m_pD3D->EnumAdapterModes( iAdapter, iMode, &DisplayMode );
			   ...
			   sprintf(tempText, "   Mode: %04dx%04d, %04dhz, Format %03d\n",
				      DisplayMode.Width,
				      DisplayMode.Height,
        ...
        for( UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++ )
        {
            ...
            for( DWORD f=0; f<dwNumFormats; f++ )
            {
                ...
                if( FAILED( m_pD3D->CheckDeviceType( iAdapter, pDevice->DeviceType,
                    ...
                    continue;
					 ...
					 sprintf(tempText, "      Valid Format: %d\n", formats[f]);
			       aLog.Log(tempText);
                ...
                if( pDevice->DeviceType == D3DDEVTYPE_HAL )
                {
                    ...
                    if( pDevice->d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED )
                    {
                        ...
                        bHALIsWindowedCompatible = TRUE;
							   sprintf(tempText, "      ***Windowed mode is possible\n");
							   aLog.Log(tempText);
                        ...
                        if( f == 0 )
                        {
                            ...
                            bHALIsDesktopCompatible = TRUE;
									 sprintf(tempText, "      ***Windowed mode is possible for current desktop\n");
									 aLog.Log(tempText);
                ...
                if( pDevice->d3dCaps.DevCaps&D3DDEVCAPS_HWTRANSFORMANDLIGHT )
                {
                    if( pDevice->d3dCaps.DevCaps&D3DDEVCAPS_PUREDEVICE )
                    {
                        ...
                        if( SUCCEEDED( ConfirmDevice( &pDevice->d3dCaps, dwBehavior[f],
                            ...
                            bFormatConfirmed[f] = TRUE;
									 sprintf(tempText, "      D3DCREATE_HARDWARE_VERTEXPROCESSING + D3DCREATE_PUREDEVICE\n");
									 aLog.Log(tempText);
                    ...
                    if ( FALSE == bFormatConfirmed[f] )
                    {
                        ...
                        if( SUCCEEDED( ConfirmDevice( &pDevice->d3dCaps, dwBehavior[f],
								...
								{
                            bFormatConfirmed[f] = TRUE;
									 sprintf(tempText, "      D3DCREATE_HARDWARE_VERTEXPROCESSING\n");
									 aLog.Log(tempText);
								}
                    }
                    ...
                    if ( FALSE == bFormatConfirmed[f] )
                    {
                        ...
                        if( SUCCEEDED( ConfirmDevice( &pDevice->d3dCaps, dwBehavior[f],
								...
								{
                            bFormatConfirmed[f] = TRUE;
									 sprintf(tempText, "      D3DCREATE_MIXED_VERTEXPROCESSING");
									 aLog.Log(tempText);
								}
                    }
                }
                ...
                if( FALSE == bFormatConfirmed[f] )
                {
                    ...
                    if( SUCCEEDED( ConfirmDevice( &pDevice->d3dCaps, dwBehavior[f],
						  ...
						  {
                        bFormatConfirmed[f] = TRUE;
								sprintf(tempText, "      D3DCREATE_SOFTWARE_VERTEXPROCESSING");
								aLog.Log(tempText);
						  }
                }
                ...
                if( bFormatConfirmed[f] && m_bUseDepthBuffer )
                {
                        ...
                        formats[f], &fmtDepthStencil[f] ) )
                    {
                        bFormatConfirmed[f] = FALSE;
						  ...
						  {
								sprintf(tempText, "      Depth Stencil Format %d\n", fmtDepthStencil[f]);
								aLog.Log(tempText);
                    }
                }
            }
            ...
            // device's list of valid modes
				aLog.Log("\n      So valid modes for this device are:\n\n");
                    ...
                    if( modes[m].Format == formats[f] )
                    {
                        if( bFormatConfirmed[f] == TRUE )
                        {
                            ...
                            pDevice->modes[pDevice->dwNumModes].DepthStencilFormat = fmtDepthStencil[f];
									 ...
									 sprintf(tempText, "      Mode: %04dx%04d, Screen Format %03d, Stencil Format %03d, behavior flags %x\n",
...
//                if( pDevice->modes[m].Width==640 && pDevice->modes[m].Height==480 )
                if( pDevice->modes[m].Width  == m_dwCreationWidth && 
						  pDevice->modes[m].Height == m_dwCreationHeight )
                {
            ...
            if( bFormatConfirmed[0] && (pDevice->d3dCaps.Caps2 & D3DCAPS2_CANRENDERWINDOWED) )
            {
                ...
                pDevice->bWindowed      = startInWindow;
            }
    ...
    for( DWORD a=0; a<m_dwNumAdapters; a++ )
    {
        ...
        {
            if( !startInWindow || m_Adapters[a].devices[d].bWindowed )
            {
                m_Adapters[a].dwCurrentDevice = d;
                m_dwAdapter = a;
                m_bWindowed = startInWindow;
...
//-----------------------------------------------------------------------------
BOOL CALLBACK CD3DApplication::SelectDeviceProc( HWND hDlg, UINT msg,
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
        ...
        default:
            _tcscpy( strMsg, _T("Undocumented application error.\n") );
    }

    if( MSGERR_APPMUSTEXIT == dwType )
    {
        _tcscat( strMsg, _T("\n\nSorry, but this game won't run.") );
...
//-----------------------------------------------------------------------------
//void CD3DApplication::StoreModeInfo( HRESULT hr, DWORD dwType )
//{
//extern void LogOutput(char *logFileName, char *text);
