...
//******************************************************************
BaseGenMode::BaseGenMode(int doid, char *doname) : GameMode(doid,doname)
{
...
//******************************************************************
BaseGenMode::~BaseGenMode()
{

}

//******************************************************************
int BaseGenMode::Init(void)  // do this when instantiated.
{
	return(0);
}

//******************************************************************
int BaseGenMode::Activate(void) // do this when the mode becomes the forground mode.
{
...
//******************************************************************
int BaseGenMode::Deactivate(void) // do this when the mode gets pushed behind another mode.
{
//	SAFE_RELEASE(g_pVB);
	for (int i = 0; i < numOfTextures; i++)
	{
...
//   puma->m_pd3dDevice->SetTransform( D3DTS_WORLD, &matWorld );
   ...
   font->DrawText( titleRect, NULL, color, "Thom's Random Spaceship BaseGen V2.0" );
...
//	HRESULT hr = font->DrawText("BaseGen string!", strlen("BaseGen string!"), &tempRect,	DT_LEFT, color);
...
//******************************************************************
void BaseGenMode::GenerateShip(int targetVerts)
{
	SAFE_DELETE(currentShip);
...
//	for (int i = 0; i < numOfParts; i++)
	while (verts < targetVerts)
	{
		...
		pm->Shift(puma->m_pd3dDevice, rnd(-10.0f,10.0f), rnd(-20.0f,20.0f), rnd(-10.0f,10.0f));
		PumaMesh *pm2 = pm->MakeCopy(puma->m_pd3dDevice);
		pm2->Mirror(puma->m_pd3dDevice);

		pm->Assimilate(puma->m_pd3dDevice,pm2);
		SAFE_DELETE(pm2);

		pm->Shift(puma->m_pd3dDevice, 0.0f,0,-20.0f);

		// make second quarter, 90 degrees off
		pm2 = pm->MakeCopy(puma->m_pd3dDevice);
		pm2->Rotate90(puma->m_pd3dDevice);

		pm->Assimilate(puma->m_pd3dDevice,pm2);
		SAFE_DELETE(pm2);

		// make the last two quarters as copies
		pm2 = pm->MakeCopy(puma->m_pd3dDevice);
		pm2->Rotate90(puma->m_pd3dDevice);
		pm2->Rotate90(puma->m_pd3dDevice);
