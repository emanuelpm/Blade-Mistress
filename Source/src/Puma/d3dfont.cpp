...
{
   FONT3DVERTEX v;   v.p = p;   v.n = n;   v.tu = tu;   v.tv = tv; v.color = color;
   return v;
}
...
//                                          D3DPOOL_DEFAULT, &m_pVB ) ) )
   ...
   for( UINT which=0; which<2; which++ )
   {
      ...
      int res = m_pd3dDevice->SetTexture( 0, m_pTexture );
		assert(D3D_OK == res);
...
// Desc: Get the dimensions of a text string
//-----------------------------------------------------------------------------
int CD3DFont::GetTextFitNum( TCHAR* strText, float size, int wordWrap )
{
   if( NULL==strText)
      return 0;
                          ...
                          TCHAR* strText, DWORD dwFlags )
{
   ...
   m_pVB->Lock( 0, 0, (BYTE**)&pVertices, D3DLOCK_NONE_THOM );
...
// Desc: Draws 2D text
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawText( RECT rect, RECT *clipRect, DWORD dwColor,
                     TCHAR* strText, DWORD dwFlags )
{
   if( m_pd3dDevice == NULL )
      return E_FAIL;
   ...
   if( dwFlags & D3DFONT_CENTERED || dwFlags & D3DFONT_VERTCENTERED)
   {
      SIZE sz;
      GetTextExtent( strText, &sz );

	   if( dwFlags & D3DFONT_CENTERED)
			sx += (rect.right - rect.left - sz.cx) / 2;
	   if( dwFlags & D3DFONT_VERTCENTERED)
			sy += (rect.bottom - rect.top - sz.cy) / 2;
   }

   if( dwFlags & D3DFONT_RIGHTSIDE)
   {
      SIZE sz;
      GetTextExtent( strText, &sz );

		sx += (rect.right - rect.left - sz.cx);
   }
   ...
   while( *strText )
   {
      ...
      FLOAT textelW = (tx2-tx1);
      FLOAT textelH = (ty2-ty1);
//      --h;
		...
		int skipDrawingLetter = FALSE;

		RECT drawRect = rect;
		if (clipRect)
		{
			if (clipRect->left > drawRect.left)
				drawRect.left = clipRect->left;
			if (clipRect->right < drawRect.right)
				drawRect.right = clipRect->right;

			if (clipRect->top > drawRect.top)
				drawRect.top = clipRect->top;
			if (clipRect->bottom < drawRect.bottom)
				drawRect.bottom = clipRect->bottom;
		}

		if (drawRect.left >= right ||
			 drawRect.right <= left ||
			 drawRect.top >= bottom ||
			 drawRect.bottom <= top )
			skipDrawingLetter = TRUE;
		else
		{
			float delta;
			if (drawRect.left > left)
			{
//				delta = drawRect.right - drawRect.left - left;
				tx1 += textelW * (drawRect.left - left) / w;
				left = drawRect.left;
			}
			if (drawRect.right < right)
			{
				tx2 -= textelW * (right - drawRect.right) / w;
				right = drawRect.right;
			}
			if (drawRect.top > top)
			{
				ty1 += textelH * (drawRect.top - top) / h;
				top = drawRect.top;
			}
			if (drawRect.bottom < bottom)
			{
				ty2 -= textelH * (bottom - drawRect.bottom) / h;
				bottom = drawRect.bottom;
			}
		}

		if (!skipDrawingLetter)
		{
			*pVertices++ = InitFont2DVertex( D3DXVECTOR4(left -0.5f,bottom-0.5f,0.9f,1.0f), dwColor, tx1, ty2 );
			*pVertices++ = InitFont2DVertex( D3DXVECTOR4(left -0.5f,top   -0.5f,0.9f,1.0f), dwColor, tx1, ty1 );
			*pVertices++ = InitFont2DVertex( D3DXVECTOR4(right-0.5f,bottom-0.5f,0.9f,1.0f), dwColor, tx2, ty2 );
			*pVertices++ = InitFont2DVertex( D3DXVECTOR4(right-0.5f,top   -0.5f,0.9f,1.0f), dwColor, tx2, ty1 );
			*pVertices++ = InitFont2DVertex( D3DXVECTOR4(right-0.5f,bottom-0.5f,0.9f,1.0f), dwColor, tx2, ty2 );
			*pVertices++ = InitFont2DVertex( D3DXVECTOR4(left -0.5f,top   -0.5f,0.9f,1.0f), dwColor, tx1, ty1 );
			dwNumTriangles += 2;
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::Render3DText( TCHAR* strText, DWORD dwFlags, DWORD dwColor )
{
   if( m_pd3dDevice == NULL )
      return E_FAIL;
   ...
   if( dwFlags & D3DFONT_CENTERED )
   {
      SIZE sz;
      GetTextExtent( strText, &sz );
      x = -(((FLOAT)sz.cx)/(10.0f * m_fTextScale) )/2.0f;
      y = -(((FLOAT)sz.cy)/(10.0f * m_fTextScale) )/2.0f;
   }
...
//-----------------------------------------------------------------------------
int CD3DFont::RealTextHeight(void)
{
	return (m_fTexCoords[0][3]-m_fTexCoords[0][1])*m_dwTexHeight;
}
